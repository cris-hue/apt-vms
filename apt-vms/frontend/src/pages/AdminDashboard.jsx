...
const AdminDashboard = () => {
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC] font-sans text-slate-900 text-left">
      ...
      <aside className="w-80 bg-slate-900 text-slate-300 flex flex-col sticky top-0 h-screen shadow-2xl">
        ...
        <nav className="flex-1 px-4 space-y-1.5 mt-6">
          ...
          <NavItem active={activeTab === 'pending'} onClick={() => setActiveTab('pending')} icon={<UserPlus size={18}/>} label="Requests" />
          <NavItem active={activeTab === 'tenants'} onClick={() => setActiveTab('tenants')} icon={<Building2 size={18}/>} label="Tenants" />
          <NavItem active={activeTab === 'guards'} onClick={() => setActiveTab('guards')} icon={<Users size={18}/>} label="Guard Team" />
          <p className="px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 mt-8 mb-4">Monitoring</p>
          <NavItem active={activeTab === 'logs'} onClick={() => setActiveTab('logs')} icon={<ClipboardList size={18}/>} label="System Logs" />
        </nav>
      ...
      <main className="flex-1 p-12 overflow-y-auto">
        <header className="flex justify-between items-center mb-10">
          <div>
            <h2 className="text-4xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">{activeTab.replace('pending', 'Authorization').replace('tenants', 'Residents').replace('guards', 'Security')}</h2>
          ...
          <button onClick={fetchData} className="p-4 bg-white border border-slate-200 rounded-2xl hover:shadow-xl transition-all text-slate-600 active:scale-95">
        ...
        <div className="bg-white rounded-[3rem] shadow-sm border border-slate-200 overflow-hidden min-h-[400px] relative text-left">
          {loading ? (
            ...
            <div className="py-32 text-center flex flex-col items-center gap-4">
               ...
               <h4 className="text-xl font-black uppercase text-slate-800 tracking-tight">Access Interrupted</h4>
               <p className="text-slate-400 font-bold text-sm max-w-md">{error}</p>
               <button onClick={fetchData} className="mt-4 bg-slate-900 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs tracking-widest">Retry Sync</button>
            </div>
          ) : (
            <table className="w-full text-left">
              <thead className="bg-slate-50/50 border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Status / Role</th>
                  <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {data.length === 0 ? (
                  <tr><td colSpan="3" className="py-32 text-center text-slate-300 font-black uppercase text-[10px] tracking-widest italic">No records found</td></tr>
                ) : (
                  data.map((item) => (
                    <tr key={item._id} className="hover:bg-blue-50/20 transition-colors group">
                      <td className="px-10 py-7">
                        <p className="font-black text-slate-800 uppercase tracking-tight group-hover:text-blue-600 transition-colors">{item.name}</p>
                        <p className="text-[11px] text-slate-400 font-bold mt-0.5 italic">{item.email || item.phone}</p>
                      </td>
                      <td className="px-10 py-7 text-center">
                        <span className="bg-blue-50 text-blue-600 border border-blue-100 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest italic">
                          {item.role} {item.unitNumber ? `• Unit ${item.unitNumber}` : ''}
                        </span>
                      </td>
                      <td className="px-10 py-7 text-right">
                        {activeTab === 'pending' ? (
                          <button onClick={() => handleApprove(item._id)} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg">Approve Access</button>
                        ) : (
                          <button onClick={() => activeTab === 'logs' ? setSelectedLog(item) : setSelectedUser(item)} className="p-3 bg-slate-50 text-slate-300 hover:text-blue-600 hover:bg-white hover:shadow-md rounded-xl transition-all">
                            {activeTab === 'logs' ? <Info size={20}/> : <ChevronRight size={20}/>}
                          </button>
                        )}
                      </td>
                    </tr>
      ...
      {selectedUser && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-slate-900/80 backdrop-blur-md">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-12 relative shadow-2xl text-left animate-in zoom-in-95 duration-200">
            <button onClick={() => setSelectedUser(null)} className="absolute top-10 right-10 text-slate-300 hover:text-slate-600 transition-colors"><X size={28}/></button>
            <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter mb-2">{selectedUser.name}</h3>
            <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-8 italic">Account Management</p>
            ...
            <button onClick={() => handleRevoke(selectedUser._id, selectedUser.name)} className="w-full bg-red-50 text-red-600 py-5 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-red-600 hover:text-white transition-all shadow-sm">Revoke System Access</button>
          </div>
        </div>
      )}
      ...
      {selectedLog && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-slate-900/80 backdrop-blur-md">
          <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl p-12 text-left animate-in zoom-in-95 duration-200 relative">
            <button onClick={() => setSelectedLog(null)} className="absolute top-10 right-10 text-slate-300 hover:text-slate-900"><X size={28}/></button>
            <div className="mb-10 text-center">
               <div className="w-20 h-20 bg-blue-600 text-white rounded-3xl flex items-center justify-center text-3xl font-black mx-auto mb-6 shadow-xl">{selectedLog.name.charAt(0)}</div>
               <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">{selectedLog.name}</h3>
            ...
            <button onClick={() => setSelectedLog(null)} className="w-full mt-10 bg-slate-900 text-white py-5 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-slate-800 transition-all shadow-xl">Dismiss Log</button>
          </div>
        </div>
      )}
    </div>
